...
/**
 * Meta's inbound message `type` → the shared `NormalizedContentType`
 * vocabulary in inbound-core.ts. Stickers are images under the hood;
 * anything else unrecognized falls back to 'text' (parseMessageContent
 * already set a `[Unsupported message type: ...]` placeholder for it).
 */
function mapMetaTypeToContentType(metaType: string): NormalizedContentType {
  switch (metaType) {
    case 'text':
    case 'image':
    case 'document':
...
/**
 * Parse Meta's raw payload into the shared `NormalizedInboundMessage`
 * shape and hand off to `processInboundMessage` (inbound-core.ts) —
 * contact/conversation dedup, persistence, and Flows/Automations/AI/
 * webhook dispatch are identical for every provider and live there.
 * The only Meta-specific work left in this function is `type` mapping
 * and, for content messages, resolving Meta's media ids via
 * `parseMessageContent` (below), which needs the decrypted access
 * token this webhook already has on hand.
 */
async function processMessage(
  ...
  // Reactions skip parseMessageContent entirely — no point resolving
  // a media id for a message type that never carries one, and it
  // keeps this the same "no unnecessary Meta media-verification call"
  // shape the original inline implementation had.
  if (message.type === 'reaction') {
    ...
    await processInboundMessage({
      ...
      message: {
        ...
        reaction: { targetExternalId: message.reaction.message_id, emoji: message.reaction.emoji },
      },
    })
    return
  }
  ...
  // `mediaType` is intentionally unused beyond parseMessageContent's own
  // proxy-URL construction — the messages table has no media_type column.
  void mediaType
