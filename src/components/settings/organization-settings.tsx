...
export function OrganizationSettings() {
  ...
  async function handleRemove() {
    ...
    if (removeConfirmText.trim() !== removingAccount.name) return;
    setRemoving(true);
    try {
      ...
      if (data.authDeleteFailed) {
        toast.warning(t('removeAuthWarningToast', { name: removingAccount.name }));
      } else {
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          ...
          <div className="space-y-2 py-2">
            <Label className="text-muted-foreground">
              {t('removeConfirmLabel', { name: removingAccount?.name || '' })}
            </Label>
            <Input
          ...
          <DialogFooter className="bg-popover border-border">
            ...
            <Button
              ...
              disabled={removing || removeConfirmText.trim() !== removingAccount?.name}
